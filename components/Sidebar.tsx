    ...
    <aside className="space-y-10">
      <div className="bg-secondary text-white p-6 rounded-sm shadow-lg">
        ...
        <form className="space-y-3" onSubmit={(e) => e.preventDefault()}>
          <input type="email" placeholder={t.placeholder} className="w-full px-4 py-2 bg-gray-800 border border-gray-700 text-white rounded-sm text-sm focus:outline-none focus:border-primary transition-colors" />
            ...
            return (
              <div key={post.id} className="flex items-start space-x-4 cursor-pointer group" onClick={() => onPostClick(post.id)}>
                <div className="flex-shrink-0 w-16 h-16 bg-gray-200 relative overflow-hidden rounded-sm">
                  <img src={post.image || (post as any).imageUrl} alt={info.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  <span className="absolute top-0 left-0 bg-primary text-white text-[9px] px-1 font-bold">#{i + 1}</span>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-primary uppercase mb-1 block">{labels[post.category] || post.category}</span>
                  <h4 className="text-sm font-bold leading-tight group-hover:text-primary transition-colors line-clamp-2 font-serif">{info.title}</h4>
                </div>
              </div>
            );
          })}
      ...
      <section>
        ...
        <div className="space-y-5">
          {recentPosts.map((post) => {
            const info = post.translations[currentLang] || post.translations.en;
            return (
              <div key={`recent-${post.id}`} className="group cursor-pointer border-b border-gray-100 pb-4 last:border-0" onClick={() => onPostClick(post.id)}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{labels[post.category] || post.category}</span>
                  <span className="text-[9px] font-bold text-gray-400 uppercase">{post.date}</span>
                </div>
                <h4 className="text-sm font-bold leading-snug group-hover:text-primary transition-colors line-clamp-2 font-serif">{info.title}</h4>
              </div>
            );
          })}
        </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-2">
          <a href="#" className="flex items-center justify-center space-x-2 py-2 bg-[#1877F2] text-white text-[10px] font-bold uppercase rounded-sm hover:opacity-90"><Facebook size={12} /> <span>Facebook</span></a>
          <a href="#" className="flex items-center justify-center space-x-2 py-2 bg-[#1DA1F2] text-white text-[10px] font-bold uppercase rounded-sm hover:opacity-90"><Twitter size={12} /> <span>Twitter</span></a>
          <a href="#" className="flex items-center justify-center space-x-2 py-2 bg-[#E4405F] text-white text-[10px] font-bold uppercase rounded-sm hover:opacity-90"><Instagram size={12} /> <span>Instagram</span></a>
          <a href="#" className="flex items-center justify-center space-x-2 py-2 bg-[#FF0000] text-white text-[10px] font-bold uppercase rounded-sm hover:opacity-90"><Youtube size={12} /> <span>Youtube</span></a>
        </div>
      </section>
